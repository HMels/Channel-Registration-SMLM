...
if True: #% Load Excel Niekamp test clusters
    ...
    DS2.find_neighbours(maxDistance=maxDistance, FrameLinking=True)
    
    ## optimization params
    learning_rates = [1e3, .1, 1e-5]
    epochs = [None, None, 100]
...
if False: #% DNA-PAINT
    ...
    #DS1.link_dataset(maxDistance=maxDistance)
    DS1.SplitBatches(Nbatches=100, FrameLinking=True) 
    DS1=DS1.SubsetFrames(0,10)
    DS1.find_neighbours(maxDistance=maxDistance, FrameLinking=True)
    #DS1, DS2=DS1.SplitDataset()   
...
DS1.ErrorDistribution_xy(nbins=nbins, xlim=xlim, error=None, fit_data=True)
...
if DS2 is not None:
    ...
    DS2.ErrorDistribution_r(nbins=nbins, xlim=xlim, error=DS2.loc_error, mu=DS1.mu)
