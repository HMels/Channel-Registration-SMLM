...
def ErrorDist(popt, N, xlim=31, error=None, fig=None):
    ...
    if fig is None: fig=plt.figure(figsize=(3.90, 3.5))
...
fig5,ax5=ErrorDist(np.average(sigma_fig5,axis=1), np.average(N_fig5,axis=1), xlim=pair_filter[2], error=np.sqrt(2)*loc_error)
