...
if False: ## Grid
    ...
    DS01=SplinesDeform(DS01, gridsize=1.5*gridsize, random_error=random_error)
...
def ErrorDistribution_r(DS1, simple_error=False, nbins=100, GaussianFit=True):
    ...
    if simple_error:
        ...
        popt,pcov=curve_fit(func, xn, n_dists[0]/N, p0=[np.std(xn), np.average(xn)])
        return popt, pcov
...
popt0, cov0 = ErrorDistribution_r(DS01, GaussianFit=True)
popt1, cov1 = ErrorDistribution_r(DS02, GaussianFit=True)
...
for i in range(len(opts)):
    for j in range(1,len(epochs_fig2)):
        ...
        try:
            ...
            popt, cov = ErrorDistribution_r(DS1, GaussianFit=True)
            sigma1_fig2[i,j]=(popt[0])
            mu1_fig2[i,j]=(popt[1])
            
            DS2.Filter(pair_filter[1])
            popt, cov = ErrorDistribution_r(DS2, GaussianFit=True)
...
NN=1
learning_rates = np.array([1.12883789e-03])

sigma1_fig3,sigma2_fig3=(np.zeros(NN, dtype=np.float32),np.zeros(NN, dtype=np.float32))
mu1_fig3,mu2_fig3=(np.zeros(NN, dtype=np.float32),np.zeros(NN, dtype=np.float32))
for i in range(NN):
    DS1=copy.deepcopy(DS01)
    DS2=copy.deepcopy(DS02)
        
    #try:
    DS1.Train_Splines(learning_rates[i], epochs, gridsize, edge_grids=1)
    DS1.Apply_Splines() 
    DS2.copy_models(DS1)
    DS2.Apply_Splines() 
    
    DS1.Filter(pair_filter[1])
    popt, cov = ErrorDistribution_r(DS1, GaussianFit=GaussianFit)
    sigma1_fig3[i]=popt[0]
    mu1_fig3[i]=popt[1]
                
    DS2.Filter(pair_filter[1])
    popt, cov = ErrorDistribution_r(DS2, GaussianFit=GaussianFit)
    sigma2_fig3[i]=popt[0]
    mu2_fig3[i]=popt[1]
    '''
    except:
        sigma1_fig3[i]=100
...
np.savetxt(output_path+'mu1_fig3.txt',mu1_fig3)
np.savetxt(output_path+'mu2_fig3.txt',mu2_fig3)
'''
'''
#######################################################################################################################
#######################################################################################################################
...
mu1_fig4,mu2_fig4=(np.zeros(len(gridsizes),dtype=float),np.zeros(len(gridsizes),dtype=float))

for i in range(len(gridsizes)):
    DS1=copy.deepcopy(DS01)
    DS2=copy.deepcopy(DS02)
    try:         
        DS1.Train_Splines(learning_rate, epochs, gridsizes[i], edge_grids=1)
        DS1.Apply_Splines() 
        DS2.copy_models(DS1)
        DS2.Apply_Splines()
        ...
        DS1.Filter(pair_filter[1])
        popt, cov=ErrorDistribution_r(DS1, GaussianFit=GaussianFit)
        sigma1_fig4[i]=popt[0]
        mu1_fig4[i]=popt[1]
        
        DS2.Filter(pair_filter[1])
        popt, cov=ErrorDistribution_r(DS2, GaussianFit=GaussianFit)
        sigma2_fig4[i]=popt[0]
        mu2_fig4[i]=popt[1]
    except:
        sigma1_fig4[i]=100
        sigma2_fig4[i]=100
...
np.savetxt(output_path+'mu1_fig4.txt',mu1_fig4)
np.savetxt(output_path+'mu2_fig4.txt',mu2_fig4)
