...
class AlignModel(Plot):
    ...
    '''
    def __init__(self, subset, developer_mode=False):
        ## Models
    ...
    # loading subset of dataset
        ...
        #mask1 = np.zeros(self.ch1.pos.shape[0], dtype=bool)
        #mask2 = mask1
    ...
    # The training loop of the model
        if not self.coupled and not self.Neighbours: raise Exception('Dataset is not coupled but no Neighbours have been generated yet')
    ...
    # Creates a grid for the Splines. The outer two gridpoints are used as anchor
        ...
        # also for NN
        if self.Neighbours:
            self.CP_idx_NN = tf.cast(tf.stack(
                [(NN2_input[:,:,0]-self.x1_min+self.edge_grids)//1, (NN2_input[:,:,1]-self.x2_min+self.edge_grids)//1]
                , axis=2), dtype=tf.int32)
        else: self.CP_idx_nn=None
    ...
    # gridsize the size of the Spline grids and edge_grids the number of gridpoints extra at the edge
        ...
        self.SplinesModel=SplinesModel(self.CP_locs, self.CP_idx, self.CP_idx_NN, direct=self.coupled)
        opt=tf.optimizers.Adagrad(lr)
        ...
        if self.coupled:
            ...
            self.SplinesModel = self.train_model(
                self.SplinesModel, Nit, opt, 
                ch1_tf=tf.Variable(self.NN1/gridsize, trainable=False),
                ch2_tf=tf.Variable(self.NN2/gridsize, trainable=False)
                )
    ...
    def plot_SplineGrid(self, ch1=None, ch2=None, ch2_original=None, d_grid=.1, lines_per_CP=1, 
        ...
        x1_grid = tf.range(self.x1_min, self.x1_max+1, d_grid, dtype=tf.float32)
        ...
        x2_grid = tf.range(self.x2_min, self.x2_max+1, d_grid, dtype=tf.float32)
